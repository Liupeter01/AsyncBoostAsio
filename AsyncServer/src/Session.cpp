...
BoostAsioProject::Session::Session(boost::asio::io_context& ioc, AsyncServer* server)
          :_socket(ioc), _server(server), _b_head_parse(false){
          boost::uuids::uuid uuid = boost::uuids::random_generator()();
          _uuid_str = boost::uuids::to_string(uuid);
          _recv_head_node = std::make_shared<BoostAsioProject::MsgHead>(HEAD_TOTAL_LENGTH);
}

std::string & BoostAsioProject::Session::GetUuid()
{
          return _uuid_str;
}

boost::asio::ip::tcp::socket& BoostAsioProject::Session::Socket() 
{
          return _socket;
}

void BoostAsioProject::Session::Start() {
...
void  BoostAsioProject::Session::Send(std::string str, short msg_id)
{
          Send(str.c_str(), str.length(), msg_id);
}

void BoostAsioProject::Session::Send(const char* msg, std::size_t max_length, short msg_id) {
          ...
          _send_queue.emplace(std::make_shared<BoostAsioProject::SendNode>(msg, max_length, msg_id));
          if (send_que_size > 0) {
                    return;
          }
...
void BoostAsioProject::Session::handle_write(std::shared_ptr<BoostAsioProject::Session> _self_shared, boost::system::error_code error) {
          if (error) {
...
void BoostAsioProject::Session::handle_read(std::shared_ptr<BoostAsioProject::Session> _self_shared, boost::system::error_code error, std::size_t bytes_transferred){
          if (error) {
          ...
          while (bytes_transferred > 0) {                       //still got some unprocessed data
                    ...
                    if (!_b_head_parse) {
                              ...
                              _recv_msg_node = std::make_shared<BoostAsioProject::RecvNode>(data_length, msg_id);
                              ...
                              BoostAsioProject::SyncLogic::getInstance()->commitPairToQueue(
                                        std::make_shared<BoostAsioProject::LogicPair>(shared_from_this(), _recv_msg_node)
                              );
