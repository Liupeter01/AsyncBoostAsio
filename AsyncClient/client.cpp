		  ...
		  try
		  {
					...
					std::thread send_thread([&sock]() {
							  for (;;) {
										...
										short origin_length = request.length();
										short request_length = boost::asio::detail::socket_ops::host_to_network_short(origin_length);

										char send_data[MAX_LENGTH]{ 0 };
										std::memcpy(send_data, &request_length, HEAD_LENGTH);
										std::memcpy(send_data + HEAD_LENGTH, request.c_str(), origin_length);

										boost::asio::write(sock, boost::asio::buffer(send_data, origin_length + HEAD_LENGTH));
					...
					std::thread recv_thread([&sock]() {
							  for (;;) {
										...
										short  origin_msg_length = boost::asio::detail::socket_ops::network_to_host_short(*(short*)reply_head);
										std::size_t replay_msg_length = boost::asio::read(sock, boost::asio::buffer(reply_msg, origin_msg_length));

										book bookdata;
										bookdata.ParseFromArray(reply_msg, origin_msg_length);
										std::cout << "Reply Msg is : " << bookdata.name() << " " << bookdata.price() << " " << bookdata.pages() << "\n";
							  }
					});
					send_thread.join();
					recv_thread.join();
